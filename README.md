...
- `/sources` — включить или отключить уведомления по каждому источнику;
- `/latest` — показать последние 10 подходящих вакансий выбранного источника.
...
Настройки источников также сохраняются в PostgreSQL. Новый источник по умолчанию
включён. Отключение останавливает только новые уведомления: бот продолжает проверять
источник и сохранять подходящие вакансии для команды `/latest`. Поэтому после
повторного включения источника старые вакансии не отправляются массово.
После первого обновления `/latest` также использует ранее отправленные вакансии из
существующей базы, поэтому история не начинается с нуля.
Дата в карточке берётся только из самого источника. Повторный парсинг обновляет дату
и остальные поля существующей вакансии, не отправляя её повторно. Если источник уже
не позволяет восстановить дату старой публикации, бот показывает «Не указана» вместо
даты обработки или отправки.
